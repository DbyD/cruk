...
function autoApproveIndividualAward($id)
{
	...
	//now let's get all work awards from the database
	$workawards = $db->prepare("SELECT * FROM tblworkawards");
	$workawards->execute();
	while($workaward = $workawards->fetch(PDO::FETCH_OBJ))
	{
		$awardOptions .= ",".$workaward->type;
	}

	//remove last ; from the string
	$awardOptions = substr($awardOptions, 0, -1);

	$nomination = $db->prepare(
	"SELECT CONCAT(approver.Fname, ' ', approver.Sname) as approverFullname,
			...
			WHERE n.id LIKE :id");
	$nomination->execute(array(":id" => $id));
	$nomination = $nomination->fetch(PDO::FETCH_OBJ);

	//Let's hook the approver up with his stuff
	$approver_name = $nomination->approverFullname;
	$approver_fname = $nomination->approverFname;
	$approver_email = $nomination->approverEaddress;
	...
	// If the nominee is offline, send an email to the xexec admin
	// else send him an email
...
/*
function individualAwardsNotUsed($id)
{
	...
	$sendEmail->Content = "<p>Dear ".$nomination->LMFname."</p>
							...
							<p>".$nomination->nomineeFname." was also given 'A Little Extra' as part of the award and can choose from: ".$awardOptions.". This was approved by ".$nomination->approverFullname.".</p>
